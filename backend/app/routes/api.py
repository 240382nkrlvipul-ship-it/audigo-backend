...
async def process_job_task(job_id: str, output_format: str, bitrate: str = None):
    ...
    if is_url:
        ...
        # Check for cookies file or environment variable and auto-convert to Netscape
        cookie_candidates = [
            os.path.join(os.path.dirname(TEMP_DIR), "cookies.txt"),
            os.path.join(os.path.dirname(TEMP_DIR), "cookies.json"),
            os.path.join(TEMP_DIR, "cookies.txt"),
            os.path.join(TEMP_DIR, "cookies.json")
        ]
        
        cookie_raw_content = None
        for candidate in cookie_candidates:
            if os.path.exists(candidate):
                try:
                    with open(candidate, "r", encoding="utf-8", errors="ignore") as f:
                        cookie_raw_content = f.read().strip()
                    if cookie_raw_content:
                        break
                except Exception:
                    pass

        if not cookie_raw_content and os.environ.get("YOUTUBE_COOKIES"):
            cookie_raw_content = os.environ.get("YOUTUBE_COOKIES").strip()

        if cookie_raw_content:
            netscape_cookie_path = os.path.join(TEMP_DIR, "valid_cookies.txt")
            if cookie_raw_content.startswith("[") or cookie_raw_content.startswith("{"):
                try:
                    import json
                    cookie_data = json.loads(cookie_raw_content)
                    if isinstance(cookie_data, list):
                        lines = ["# Netscape HTTP Cookie File\n# This file was generated by AudiGo\n\n"]
                        for c in cookie_data:
                            dom = c.get("domain") or c.get("host") or ".youtube.com"
                            sub = "TRUE" if dom.startswith(".") else "FALSE"
                            p = c.get("path", "/")
                            sec = "TRUE" if c.get("secure", False) else "FALSE"
                            exp = str(int(c.get("expirationDate") or c.get("expires") or c.get("expiry") or 2147483647))
                            n = c.get("name", "")
                            v = c.get("value", "")
                            if n:
                                lines.append(f"{dom}\t{sub}\t{p}\t{sec}\t{exp}\t{n}\t{v}\n")
                        with open(netscape_cookie_path, "w", encoding="utf-8") as cf:
                            cf.writelines(lines)
                        ydl_opts['cookiefile'] = netscape_cookie_path
                except Exception:
                    with open(netscape_cookie_path, "w", encoding="utf-8") as cf:
                        cf.write(cookie_raw_content)
                    ydl_opts['cookiefile'] = netscape_cookie_path
            else:
                with open(netscape_cookie_path, "w", encoding="utf-8") as cf:
                    cf.write(cookie_raw_content)
...
async def download_job(job_id: str):
    async with AsyncSessionLocal() as db:
        ...
        custom_name = job.input_filename or f"AudiGo_{job_id}"
        clean_name = "".join(c for c in custom_name if c.isalnum() or c in (' ', '-', '_')).rstrip()
        if not clean_name:
            clean_name = f"AudiGo_{job_id}"
